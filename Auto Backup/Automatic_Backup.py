...
other folder and syncs everything which recursively lies within one of
the source folders. For files bigger than a threshold they are first
gziped."""
...
def parse_input():
    ...
    parser.add_argument('-c','--compress', nargs=1, type=int, help='Gzip threshold in bytes, Deafault 1024KB', default=[1024000]) # Default Threshold is 1024KB
    # Help is triggered when there is no Input Provided if len(sys.argv) == 1:
    parser.print_help()
    sys.exit()
...
def size_if_newer(source, target):
    """
    If there is a difference in file size, this function reports the
    difference.
    Args:
    source: Source Path
    ...
    try:
        ...
        try:
            ...
            target_ts = 0
            # The time difference of one second is necessary since subsecondaccuracy # of os.st_mtime is striped by copy2
...
def threaded_sync_file(source, target, compress):
    ...
    Args:
    source: Source Path
    target: Target for ZIP file compress: The compression threshold
    Returns: The threads
    """
    ...
    try:
        if compress:
            with gzip.open(target + '.gz', 'wb') as target_fid: 
                with open(source, 'rb') as source_fid: target_fid.writelines(source_fid) 
...
def sync_root(root, arg):
    ...
    for path, _, files in os.walk(root): 
        for source in files:
            source = path + '/' + source 
            threads.append(threaded_sync_file(source, target + source, compress)) # sync_file(source, target + source, compress) 
...
"""
Example Usage:
> python Automatic_Backup.py --target ./Backup_Folder --source ./Source_Folder 
"""