...
def todo(ctx):
    ...
    # Open todo.txt - first line contains latest ID, rest contains tasks and IDs
...
def tasks(ctx):
    if ctx.obj['TASKS']:
        # Iterate through tasks stored in context
        ctx.echo('YOUR TASKS\n*******')
...
def done(ctx, fin_taskid):
    ...
    if str(fin_taskid) in ctx.obj['TASKS'].keys():
        ...
        if ctx.obj['TASKS']:
            ...
            # Reset ID tracker to 0 if list is empty
            f.writelines([str[0] + '\n'])
    else:
